...
app = Flask(__name__)
quotes = ['Ich behandel meinen Koerper wie einen Tempel', 'Gewoehn dich dran','Ich hab Zuhause ne Chips-Zange. Wirklich!','Ein Tempel braucht auch mal eine Pizza', 'Dein Handy ist schon laecherlich dreckig', 'Kein Mensch braucht PowerPoint', 'Mit so nem Stuhl wuerde ich bei Tipico auch nur Geld verlieren', 'Ich weiss gar nicht worauf ich gerade warte... Feierabend oder so','Du machst doch eh Excel','Das ist scheissegal, ich hab eh Tabletten', 'Das ist out of Context','So viel dazu, du arbeitest den ganzen Tag','Was ist denn los mit dir? Du bist ein besserer Mensch','Ich hab Angst da vergewaltigt zu werden. Ich bin leichte Beute','Paracetamol ist guenstig, das ist der Sinn dahinter','Kannst zu RTL damit gehen','Matthias kommt zu uns. Ihr koennt eure Arbeit niederlegen']
...
def hello_world():
    ...
    if 'deal' in message:
        return jsonify({"color": "red", "message": memes[random.randint(0,len(memes)-1)], "notify": True, "message_format": "text"}),201
